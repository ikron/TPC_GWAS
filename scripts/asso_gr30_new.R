...
gr30 <- dplyr::filter(genomeans, Temp == 30)

myY <- gr30[gr30$Genot %in% geno.names,]
myY <- myY[,-c(2,3)] #Drop the temp column, not needed
colnames(myY) <- c("Taxa", "gr30")

##Run GAPIT
myGAPIT30 <- GAPIT(Y = myY, PCA.total = 4, G = myG, model = "Blink", Geno.View.output = FALSE, PCA.View.output = FALSE, Phenotype.View = FALSE)
