...
##Usage: Rscript LD_calc_fine.R <chr>
...
snpsets <- round(seq(100, nrow(myG.chr)-101, length.out = 10), 0)
...
for(i in 1:10) { #Loop over the 10 SNP sets
    ...
    myG.SNP <- myG.chr[sampledsnp:(sampledsnp+99),] #Take 100 SNPs
...
filename <- paste0("pw_LD_fine_chr", cur.chr, ".RData") #Setting up filenames

save(finalresults, file = paste0("/projappl/project_2000350/gwas/results/", filename)) #Save the data
