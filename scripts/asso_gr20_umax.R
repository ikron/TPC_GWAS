##Run GAPIT for gr20 with 4 PCs and umax as covariates
...
gr20 <- dplyr::filter(genomeans, Temp == 20)

myY <- gr20[gr20$Genot %in% geno.names,]
myY <- myY[,-c(2,3)] #Drop the temp column, not needed
colnames(myY) <- c("Taxa", "gr20")
...
##Run GAPIT
myGAPIT20 <- GAPIT(Y = myY, CV = myCV, G = myG, model = "Blink")
